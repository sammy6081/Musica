...
class MusicPlayer {
  constructor(songs) {
    ...
    this.isRandom = false;
    this.audio.addEventListener('timeupdate', () => this.updateProgressSlider());
    ...
    this.prevBtnMusicPlayer = document.querySelector('.prev-btn');
    this.nextBtnMusicPlayer = document.querySelector('.next-btn');
    this.randomBtnMusicPlayer = document.querySelector('.random-btn');

    this.playPauseBtn.addEventListener('click', () => this.togglePlayPause());
    this.volumeSlider.addEventListener('input', () => this.updateVolume());
    this.progressSlider.addEventListener('input', () => this.updateProgress());
    this.prevBtnMusicPlayer.addEventListener('click', () => this.playPrevSong());
    this.nextBtnMusicPlayer.addEventListener('click', () => this.playNextSong());
    this.randomBtnMusicPlayer.addEventListener('click', () => this.toggleRandom());
  ...
  updateSongInfo() {
    ...
    artistElement.textContent = currentSong.category;
    albumArtworkElement.src = currentSong.img;
    this.audio.src = currentSong.song;
  }

  playSong(index) {
    if (index >= 0 && index < this.songs.length) {
      this.audio.src = this.songs[index].song;
      this.audio.play();
      ...
      titleElement.textContent = this.songs[index].title;
      artistElement.textContent = this.songs[index].category;
      albumArtworkElement.src = this.songs[index].img;
  ...
  playPrevSong() {
    this.playSong((this.currentSongIndex - 1 + this.songs.length) % this.songs.length);
    if (this.audio.played) {
      this.playIcon.classList.remove('hidden');
      this.pauseIcon.classList.add('hidden');
    } else {
  ...
  updateProgress() {
    const currentTime = this.audio.duration * (this.progressSlider.value / 100);
...
document.addEventListener('DOMContentLoaded', () => {
  const player = new MusicPlayer(menu);
});
